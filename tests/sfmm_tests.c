...
//############################################
Test(sf_memsuite_student, test_1, .init = sf_mem_init, .fini = sf_mem_fini){
	void *x = sf_malloc(45);
	void *y = sf_realloc(x,32);
	cr_assert_not_null(y, "y is NULL!");
	sf_header *header = (sf_header*)((char*)y - 8);
	cr_assert(header->block_size << 4 == 64, "Expecting 64, not met");
}

Test(sf_memsuite_student, test_2, .init = sf_mem_init, .fini = sf_mem_fini){
	void *x = sf_malloc(45);
	void *b = sf_realloc(x,16);
	cr_assert_not_null(b, "y is NULL!");
	sf_header *header = (sf_header*)((char*)b - 8);
	cr_assert(header->block_size << 4 == 32, "Expecting 32, not met");
}

Test(sf_memsuite_student, test_3, .init = sf_mem_init, .fini = sf_mem_fini){
	void *x = sf_malloc(16385);
	cr_assert_null(x, "x is not NULL!");
	cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");
}

Test(sf_memsuite_student, test_4, .init = sf_mem_init, .fini = sf_mem_fini){
	sf_malloc(16336);
	void *x = sf_malloc(16);
	x = sf_realloc(x, 32);
	cr_assert_not_null(x, "x is NULL!");
...
Test(sf_memsuite_student, test_5, .init = sf_mem_init, .fini = sf_mem_fini){
